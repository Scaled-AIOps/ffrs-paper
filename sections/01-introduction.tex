...
Every software project has a channel through which stakeholders raise feature requests (FRs) and bug reports (BRs). Far fewer have a working \emph{loop}: an item that is registered, acknowledged, acted upon, and whose outcome is reported back to the person who raised it. In the author's experience across nearly three decades of projects, the failure is seldom at capture---forms, mail, tickets and trackers abound---but in the interval that follows. Items wait for a triage meeting, for a developer with capacity, for a decision; the requester hears nothing; and because that interval is unmeasured, its cost is invisible.
...
\begin{enumerate}
  ...
  \item \textbf{An open implementation} (Sec.~\ref{sec:impl}): a 5\,KB (gzipped) widget, one serverless function, and a scheduled agent, run as a shared service that any site adopts with one tag.
  \item \textbf{An evaluation} (Sec.~\ref{sec:eval}) on two sites against pre-registered targets, and an analysis of which parts of the design are load-bearing.
\end{enumerate}

...
Capture function & AWS Lambda (function URL) & Google Cloud Run / Functions, Azure Functions, Cloudflare Workers, a container behind any HTTP gateway \\
Edge / CDN path & CloudFront on the service host & Cloudflare, Fastly, or a reverse proxy in front of the site \\
...
\begin{tikzpicture}[
  ...
  \node[ours] (widget) {Widget\\edge tab, 5\,KB gz};
  \node[ext, below=of widget] (page) {\texttt{/feedback/}\\no-JS form $\cdot$ status};
  \node[ext, right=of widget] (cdn) {CDN\\service host};
...
A framework-free widget (one script tag loaded from the service host, configured by data attributes) renders an edge tab on every page inside a shadow root, so it neither inherits nor disturbs the host page's styles and works under a strict content security policy. It offers ``request a feature'' and ``report a bug'', captures the page URL and---for bugs---a client-side screenshot, and takes an optional e-mail with an explicit consent checkbox. A plain HTML form at \texttt{/feedback/} is the no-JavaScript fallback; status links land there or, for sites without such a page, on a status page served by the function.

\subsection{Tenancy}
One deployment serves many sites. A site is a \emph{tenant}: a slug, its allowed origins, its own tracker repository and token, and optional bot-check, mail and agent settings, held in the parameter store and reloaded within minutes, so onboarding needs no deploy. A request names its tenant (\texttt{data-site}) and must come from one of that tenant's origins; the closing webhook is matched to a tenant by repository and verified with that tenant's secret. Tenants share code, guards and metrics, never trackers or data.

\subsection{Tracker as system of record}
The function validates the submission, applies guards (honeypot, per-address rate limit, a bot check), stores the screenshot in a private object store, and creates the issue synchronously with labels \texttt{kind:*}, \texttt{severity:*} and a hidden marker carrying the reference id. Only then does it return \texttt{202 \{ref\}}. If the tracker is unavailable the caller receives an honest \texttt{502} and the widget retries with the same idempotency key---nothing is half-stored. Route therefore coincides with Capture; Respond and Close are read from the tracker itself (first comment or pull request, close event, \texttt{outcome:*} labels or close reason).
...
A tracker webhook fires on close; the function derives the outcome (\texttt{outcome:*} label $\succ$ close reason $\succ$ kind default) and sends the closing note. Comments and close time need no webhook---the tracker already records them. The same webhook receives new comments and removes the attribution footer that the agent's hosting tooling appends to comments it posts, so the public record carries only the agent's text and no product name.

\subsection{Agentic Respond stage}
A scheduled cloud agent runs against the tracker. For each open item without a first response, it either opens a pull request on the target repository (code path) or posts a proposal comment (proposal path). Acceptance and confirmation are ordinary comments matching a small command grammar (\texttt{/accept}, \texttt{/confirm}, \texttt{/reject <reason>}) by the requester and by a maintainer respectively; the agent executes a proposal only after a maintainer's confirmation (a requester without a tracker account cannot accept; the maintainer then acts for both). In the reference deployment the agent is a hosted coding-agent routine that runs hourly with the site and tracker repositories checked out and authenticated access to the tracker (Sec.~\ref{sec:impl}); any coding agent that can read the tracker, run the project's tests and open a pull request fits the role. Its instructions are a versioned prompt of about 850 words that fixes the selection rule (open items with a kind label and no prior response, oldest first, at most three per run), the two paths, the label protocol, a 15-minute budget with at most five minutes on test infrastructure, and the rules \emph{never push to main, never force-push a shared branch, one branch per item, clean up temporary files, never write an attribution trailer}. ``Execute'' for a static site means a pull request (content or code) or a drafted reply; the agent has no other write path. Its tracker identity is the maintainer's, so agent comments carry a fixed marker and are excluded from the human metrics.
\subsection{Security and threat model}
An agent that reads untrusted input from the public internet and changes code is exposed to indirect prompt injection: a requester could embed instructions in a description that attempt to exfiltrate secrets or introduce harmful code. \ffrs{} mitigates this structurally rather than by trusting the agent. The agent runs in an isolated sandbox that holds no production secrets (cloud credentials exist only in the infrastructure pipeline); its repository credentials allow it to push branches and open pull requests, while a repository rule blocks direct pushes to the main branch for every actor except a repository administrator; and the mandatory human checkpoint means that even a successfully injected payload cannot reach the mainline without a maintainer's review and merge. The \texttt{ffrs/*} branch naming and the \emph{never push to main} rule are additionally fixed in the versioned prompt, but the guarantee rests on the repository rule and the merge, not on the prompt.


\subsection{Operability and audit trail}
Every layer is declared in version control and changed only through it: the site and widget are built from a repository, the function is a versioned bundle, the cloud resources are infrastructure-as-code, and the agent's protocol and prompt are files in the same repositories. Infrastructure changes go through \texttt{plan}/\texttt{apply} from a reviewed commit; content and code changes---including every change the agent proposes---arrive as pull requests. Consequently the audit trail for any question (\emph{who changed what, when, and why; which items the agent touched; what a reviewer approved}) is the commit and pull-request history plus the public tracker, with no separate audit store to maintain. Three switches make \ffrs{} detachable: a build-time flag (ships zero widget bytes), a runtime kill switch in the parameter store (\texttt{503} within a minute, no deploy), and an infrastructure flag that removes every resource; each is itself a committed change. Secrets are read from the parameter store at run time and never appear in infrastructure state or in the repositories.

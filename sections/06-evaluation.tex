...
\subsection{Protocol}
The metrics and targets were fixed in the project's case-study document before the first production item on 18~August~2026, and are applied here unchanged: per-kind, per-ISO-week metrics; time to first human response under 72~h at the median; TTC for bugs under 30 days at the median; a closing notification for 100\,\% of consented closed items; signal ratio reported without a target. The case-study document predates the agentic Respond stage and calls its human-response measure ``TTFR''; in this paper's terms (Sec.~\ref{sec:model}) that measure is TTHR. TTFR including agent responses, and agent share, were added with the agent and are therefore reported without a target. The 12-week window and its read-outs are re-based to the shared deployment's T0: week~4 on 21~October~2026, week~12 on 16~December~2026. The source of truth is, for S1, the public tracker read by the published \texttt{metrics}/\texttt{export} commands and, for S2, its weekly research rows; the resulting CSVs are committed with their export date. Items filed by the maintainer to test the system are labelled \texttt{spam} in the tracker and excluded.
